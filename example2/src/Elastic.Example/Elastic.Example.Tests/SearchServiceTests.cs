...
namespace Elastic.Example.Tests
{
    ...
    public class SearchServiceTests
    {
        ...
        public void Indexing_MoviesAndActors_ShouldSucceed()
        {
            ...
            AddMovie(
                name: "ffffff ddss",
                summary: "assassin swears vengeance.",
                actors: "Ethan Hawke",
                airDate: new DateTime(2005, 10, 5));
            ...
            var result = this.Search<SearchItemDocumentBase>("Fire");
            Assert.AreEqual(2, result.Items.Count, result.DebugInformation);
            result = this.Search<MovieSearchItem>("Fire");
            Assert.AreEqual(2, result.Items.OfType<SearchItemDocumentBase>().Count(), result.DebugInformation);

            result = this.Search<SearchItemDocumentBase>("Denzel");
            Assert.AreEqual(3, result.Items.Count, result.DebugInformation);
            result = this.Search<ActorSearchItem>("Denzel");
            Assert.AreEqual(1, result.Items.OfType<SearchItemDocumentBase>().Count(), result.DebugInformation);
        }

        private SearchServiceResults Search<T>(string query) where T : SearchItemDocumentBase
        {
            return _service.Search<T>(new CommonSearchRequest()
            {
                Query = query
            });
        }
